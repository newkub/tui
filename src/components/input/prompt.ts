...
export async function selectPrompt<T>(options: {
  ...
  return new Promise((resolve) => {
    ...
    const ask = () => {
      rl.question('Select an option (1-' + options.options.length + '): ', (answer) => {
        const selected = parseInt(answer) - 1;
        if (isNaN(selected) || selected < 0 || selected >= options.options.length) {
          console.log(pc.red(`Please enter a number between 1 and ${options.options.length}`));
          return ask();
        }
        rl.close();
        resolve(options.options[selected].value);
      });
    };
